    ...
    <main className="min-h-screen font-sans transition-colors duration-1000">
      ...
      <nav className="fixed top-0 w-full z-50 px-6 py-8 flex justify-between items-center mix-blend-difference text-white">
        ...
        <div className="flex items-center gap-8">
          <a href="https://boluadeoye.com.ng" className="text-[10px] font-bold uppercase tracking-[0.3em] hover:opacity-50 transition-opacity flex items-center gap-2">
        ...
        <div className="max-w-[1440px] mx-auto w-full grid lg:grid-cols-12 gap-16 items-center">
          <div className="lg:col-span-7 space-y-10">
            <Reveal>
              ...
              <h1 className="text-4xl md:text-6xl lg:text-[90px] font-serif leading-[0.95] tracking-tighter">
            ...
            <Reveal delay={0.4}>
              <a href={whatsappUrl} className="inline-flex items-center gap-6 px-10 py-5 bg-black dark:bg-white text-white dark:text-black rounded-full text-[10px] font-bold uppercase tracking-[0.3em] hover:scale-105 transition-all duration-500 shadow-2xl">
      ...
      {/* --- 2. SELECTED WORKS (CLEAN GRID) --- */}
      <section className="py-32 px-6 md:px-12 max-w-[1440px] mx-auto">
        <Reveal><h2 className="text-3xl md:text-5xl font-serif italic mb-20">Selected Works</h2></Reveal>
        <div className="grid md:grid-cols-2 gap-12">
          {projects.map((p, i) => (
            <Reveal key={i} delay={i * 0.1}>
              <a href={p.link} target="_blank" className="group block space-y-6">
                ...
                <div className="flex justify-between items-end px-4">
                  <div>
                    <span className="text-[9px] font-bold uppercase tracking-[0.3em] opacity-30">{p.tag}</span>
                    <h3 className="text-2xl font-serif mt-1">{p.title}</h3>
                  </div>
          ...
          {[
            ...
            <Reveal key={i} delay={i * 0.2}>
              <div className="space-y-6">
                ...
                <h4 className="text-xl font-serif font-bold uppercase tracking-tighter">{item.title}</h4>
      ...
      <section className="py-32 px-6 md:px-12 max-w-3xl mx-auto">
        <Reveal><h2 className="text-3xl md:text-5xl font-serif italic text-center mb-20">Common Inquiries</h2></Reveal>
            ...
            <Reveal key={i} delay={i * 0.1}>
              <div className="border-b border-black/10 dark:border-white/10">
                <button onClick={() => setActiveFaq(activeFaq === i ? null : i)} className="w-full py-8 flex justify-between items-center text-left group">
                  <span className="text-lg font-medium group-hover:opacity-50 transition-opacity">{faq.q}</span>
      ...
      <footer className="py-12 px-6 md:px-12 border-t border-black/5 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
        <span className="text-[9px] font-bold uppercase tracking-[0.5em] opacity-20">© 2024 Adeoye Boluwatife</span>
        <div className="flex gap-8">
          <a href={whatsappUrl} className="text-[10px] font-bold uppercase tracking-[0.3em] hover:text-accent transition-colors">WhatsApp</a>
          <a href="mailto:boluadeoye97@gmail.com" className="text-[10px] font-bold uppercase tracking-[0.3em] hover:text-accent transition-colors">Email</a>
        </div>
      </footer>
    </main>
  );
