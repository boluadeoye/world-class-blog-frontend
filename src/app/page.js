      ...
      <section className="px-6 md:px-12 pb-24 relative z-10">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
            ...
            {heroPost && (
              <Link href={`/post/${heroPost.slug}`} className="md:col-span-8 group relative block min-h-[450px] rounded-2xl overflow-hidden bg-slate-900">
                {getImg(heroPost) ? (
                ...
                <div className="absolute bottom-0 left-0 p-8 md:p-12 max-w-3xl">
                  <span className="inline-block px-3 py-1 mb-4 text-xs font-bold tracking-widest text-slate-950 bg-amber-400 rounded-full">
            ...
            {/* Sub Features (Spans 4 cols) */}
            <div className="md:col-span-4 flex flex-col gap-6">
              {subFeatures.map((post) => (
                <Link key={post.slug} href={`/post/${post.slug}`} className="flex-1 relative block p-8 rounded-2xl bg-slate-900 border border-slate-800 hover:border-slate-600 transition-colors group flex flex-col justify-between">
                  <div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {recentNotes.map((post, i) => (
                <Link key={post.slug} href={`/post/${post.slug}`} className="group flex items-baseline gap-4 p-4 rounded-lg hover:bg-white/5 transition-colors border-b border-slate-800/50 last:border-0">
