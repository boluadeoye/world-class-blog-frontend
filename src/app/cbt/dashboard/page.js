...
function ExamSetupModal({ course, isPremium, onClose, onStart, onUpgrade }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in zoom-in duration-200">
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-sm overflow-hidden border border-white">
        <div className={`${isBlocked ? 'bg-red-900' : 'bg-[#004d00]'} p-5 text-white relative`}>
        ...
        <div className="p-5">
          {isBlocked ? (
            <div className="text-center py-2">
              ...
              <button onClick={onUpgrade} className="w-full py-3 bg-yellow-500 text-black rounded-xl font-black text-[10px] uppercase tracking-widest shadow-lg mb-3">Upgrade to Premium</button>
            ...
            <>
              <div className="mb-6">
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[15, 30, 45, 60].map((time) => {
                    ...
                    return (
                      <button key={time} disabled={isRestricted} onClick={() => setDuration(time)} className={`py-2.5 rounded-xl text-[10px] font-black transition-all border-2 relative overflow-hidden ${duration === time ? 'border-green-600 bg-green-50 text-green-900' : 'border-gray-100 text-gray-400 bg-gray-50'} ${isRestricted ? 'opacity-40' : ''}`}>
                ...
                {!isPremium && (
                  <button onClick={onUpgrade} className="mt-4 w-full flex items-center justify-center gap-2 text-[9px] text-yellow-700 bg-yellow-50 p-2.5 rounded-xl border border-yellow-100 hover:bg-yellow-100 transition-colors">
              ...
              <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 py-3 border-2 border-gray-100 rounded-xl text-[10px] font-black text-gray-400 uppercase tracking-widest">Cancel</button>
                <button onClick={() => onStart(duration)} className="flex-[2] py-3 bg-gray-900 text-white rounded-xl text-[10px] font-black shadow-xl hover:bg-black active:scale-95 uppercase tracking-widest flex items-center justify-center gap-2">Start <Play size={12} fill="currentColor" /></button>
              </div>
            </>
          )}
        </div>
...
/* === 2. COMPACT DISCLAIMER === */
function DisclaimerCard() {
  const [isOpen, setIsOpen] = useState(true);
  return (
    <div className="bg-[#FFF8F0] border border-orange-100 rounded-2xl overflow-hidden mb-4 shadow-sm">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-4 text-left">
        <div className="flex items-center gap-3">
          <div className="bg-orange-100 w-8 h-8 flex items-center justify-center rounded-lg text-orange-600"><Info size={18} /></div>
          <div><h3 className="font-black text-xs text-[#5A3A29] uppercase tracking-wide">Important Disclaimer</h3><p className="text-[9px] text-orange-400 font-bold">Read before starting</p></div>
        </div>
        <ChevronDown size={16} className={`text-orange-300 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      {isOpen && (
        <div className="px-4 pb-4 text-[10px] text-[#8B5E3C] leading-relaxed border-t border-orange-50 pt-3">
...
function CourseCard({ course, onLaunch, variant = "green", isPremium }) {
  ...
  const isBlocked = !isPremium && course.user_attempts >= 2;
  return (
    <div className="bg-white p-3 rounded-xl shadow-sm border border-gray-100 flex justify-between items-center group active:scale-[0.98] transition-transform mb-2">
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-black text-[10px] border ${isGst ? 'bg-green-50 text-[#004d00] border-green-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{course.code.slice(0,3)}</div>
        <div><h3 className="font-black text-gray-900 text-xs uppercase">{course.code}</h3><p className="text-[9px] text-gray-500 font-medium truncate w-32">{course.title}</p></div>
      </div>
      <button onClick={(e) => { e.preventDefault(); onLaunch(course); }} className={`w-8 h-8 rounded-full flex items-center justify-center shadow-md transition-colors z-10 ${isBlocked ? 'bg-red-50 text-red-400 border border-red-100' : isGst ? 'bg-gray-900 text-white hover:bg-[#004d00]' : 'bg-blue-900 text-white hover:bg-blue-950'}`}>{isBlocked ? <Lock size={12} /> : <Play size={12} fill="currentColor" />}</button>
    </div>
  );
}
...
export default function StudentDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#fcfdfc] font-sans text-gray-900 pb-32 relative">
      ...
      {/* === COMPACT HEADER === */}
      <header className="bg-[#004d00] text-white pt-6 pb-12 px-5 rounded-b-[30px] shadow-xl relative z-10">
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-white rounded-xl flex items-center justify-center border-2 border-white/20 shadow-lg overflow-hidden relative">
              {avatarUrl && <img src={avatarUrl} alt="Profile" className="w-full h-full object-cover" />}
              {isPremium && <div className="absolute top-0 right-0 bg-yellow-400 p-0.5 rounded-bl-md shadow-sm"><Crown size={8} className="text-black" fill="currentColor" /></div>}
            </div>
            <div>
              <p className="text-green-200 text-[9px] font-bold uppercase tracking-widest mb-0.5">{greeting}</p>
              <h1 className="text-lg font-black leading-none truncate w-32">{student.name.split(" ")[0]}</h1>
            </div>
          </div>
          <div className="flex gap-2">
            <a href="https://wa.me/2348106293674" target="_blank" rel="noopener noreferrer" className="bg-green-500 p-2.5 rounded-xl border border-white/20 text-white hover:bg-green-400 transition-all shadow-lg animate-pulse"><Headset size={16} /></a>
            <button onClick={triggerLogout} className="bg-[#006400] p-2.5 rounded-xl border border-white/10 hover:bg-red-600 transition-colors shadow-lg"><LogOut size={16} /></button>
          </div>
        </div>
      ...
      <div className="px-5 -mt-6 relative z-20 space-y-5">
        ...
        {/* === COMPACT HISTORY === */}
        <section>
          <div className="flex items-center gap-2 mb-3"><History size={16} className="text-gray-400" /><h2 className="font-black text-[11px] text-gray-500 uppercase tracking-widest">Exam History</h2></div>
          {examHistory.length > 0 ? (
              ...
              {examHistory.map((item) => {
                ...
                return (
                  <div key={item.id} className="p-3 flex items-center justify-between hover:bg-gray-50 transition-colors">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 font-black text-[9px]">{item.course_code.slice(0,3)}</div>
                    ...
                    <div className={`text-right px-3 py-1 rounded-lg border ${colorClass}`}>
                      <p className="font-black text-[10px]">{pct}%</p>
            ...
            <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm text-center"><p className="text-[9px] font-black text-gray-300 uppercase tracking-widest">No missions completed yet</p></div>
          )}
        </section>
        ...
        <section className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <button onClick={() => setGstExpanded(!gstExpanded)} className="w-full p-4 flex items-center justify-between bg-green-50/50">
            <div className="flex items-center gap-2"><BookOpen size={16} className="text-[#004d00]" /><h2 className="font-black text-[11px] text-gray-700 uppercase tracking-widest">General Studies</h2></div>
        ...
        <section className="bg-[#f0f4ff] rounded-2xl shadow-xl shadow-blue-900/5 border border-blue-100 p-4">
          <div className="flex items-center justify-between mb-3"><div className="flex items-center gap-2"><div className="bg-blue-600 p-1 rounded-lg text-white shadow-lg shadow-blue-200"><Layers size={14} /></div><h2 className="font-black text-[11px] text-blue-900 uppercase tracking-widest">Other Courses</h2></div><Sparkles size={14} className="text-blue-400 animate-pulse" /></div>
        ...
        <section>
          ...
          {leaders.length > 0 ? (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-3 overflow-x-auto flex gap-3 custom-scrollbar">
              {leaders.map((user, i) => (
                <div key={i} className="min-w-[140px] bg-gray-50 rounded-xl p-4 border border-gray-100 flex flex-col items-center text-center relative">
                  {i === 0 && <div className="absolute -top-2 -right-2 bg-yellow-400 text-white p-1 rounded-full shadow-sm"><Crown size={10} fill="currentColor" /></div>}
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs mb-2 overflow-hidden border-2 ${i === 0 ? 'border-yellow-400' : 'border-gray-100'}`}><img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${user.name.replace(/\s/g, '')}&backgroundColor=transparent`} alt={user.name} className="w-full h-full object-cover" /></div>
            ...
            <div className="text-center py-8 bg-white rounded-2xl border border-gray-100"><p className="text-gray-400 text-[9px] font-black uppercase tracking-widest">Awaiting Rankings...</p></div>
          )}
        </section>
      </div>
