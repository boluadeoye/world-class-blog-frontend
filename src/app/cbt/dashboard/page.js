  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in zoom-in duration-200">
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden border border-white">
        <div className={`${isBlocked ? 'bg-red-900' : 'bg-[#004d00]'} p-8 text-white relative`}>
          ...
          <h3 className="font-black text-sm uppercase tracking-[0.2em] flex items-center gap-2">
            {isBlocked ? <Lock size={18} /> : <Settings size={18} />} {isBlocked ? "Access Denied" : "Session Config"}
          </h3>
        ...
        <div className="p-8">
          {isBlocked ? (
            <div className="text-center py-4">
              <div className="w-16 h-16 bg-red-50 text-red-600 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-red-100"><Lock size={32} /></div>
            ...
            <>
              <div className="mb-8">
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[15, 30, 45, 60].map((time) => {
                    ...
                    return (
                      <button key={time} disabled={isRestricted} onClick={() => setDuration(time)} className={`py-4 rounded-2xl text-xs font-black transition-all relative overflow-hidden border-2 ${duration === time ? 'border-green-600 bg-green-50 text-green-900 shadow-inner' : 'border-gray-100 text-gray-400 bg-gray-50'} ${isRestricted ? 'opacity-40' : ''}`}>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => onStart(duration)} className="flex-[2] py-4 bg-gray-900 text-white rounded-2xl text-[10px] font-black shadow-xl hover:bg-black active:scale-95 uppercase tracking-widest flex items-center justify-center gap-2">Start <Play size={14} fill="currentColor" /></button>
              </div>
            </>
          )}
        </div>
...
/* === 2. DISCLAIMER ACCORDION (WARM BEIGE) === */
    ...
    <div className="bg-[#FFF8F0] border border-orange-100 rounded-3xl overflow-hidden mb-8 shadow-sm">
      <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-6 text-left">
        <div className="flex items-center gap-4">
          <div className="bg-orange-100 w-12 h-12 flex items-center justify-center rounded-full text-orange-600 shrink-0"><Info size={24} /></div>
          <div><h3 className="font-black text-sm text-[#5A3A29] uppercase tracking-wide">Important Disclaimer</h3><p className="text-[11px] text-orange-400 font-bold mt-0.5">Read before starting</p></div>
        </div>
...
function CourseCard({ course, onLaunch, variant = "green", isPremium }) {
  ...
  return (
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center group active:scale-[0.98] transition-transform mb-3">
      <div className="flex items-center gap-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black text-xs border ${isGst ? 'bg-green-50 text-[#004d00] border-green-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{course.code.slice(0,3)}</div>
        <div><h3 className="font-black text-gray-900 text-sm uppercase">{course.code}</h3><p className="text-[10px] text-gray-500 font-medium truncate w-40">{course.title}</p></div>
      </div>
      <button onClick={(e) => { e.preventDefault(); onLaunch(course); }} className={`w-10 h-10 rounded-full flex items-center justify-center shadow-lg transition-colors z-10 ${isBlocked ? 'bg-red-50 text-red-400 border border-red-100' : isGst ? 'bg-gray-900 text-white hover:bg-[#004d00]' : 'bg-blue-900 text-white hover:bg-blue-950'}`}>{isBlocked ? <Lock size={14} /> : <Play size={14} fill="currentColor" />}</button>
    </div>
  );
}
...
export default function StudentDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#fcfdfc] font-sans text-gray-900 pb-48 relative">
      ...
      {/* === SPACIOUS HEADER === */}
      <header className="bg-[#004d00] text-white pt-10 pb-24 px-8 rounded-b-[3rem] shadow-2xl relative z-10">
        <div className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-5">
            ...
            <div>
              <p className="text-green-200 text-[11px] font-bold uppercase tracking-widest mb-1">{greeting}</p>
              <h1 className="text-2xl font-black leading-none truncate w-48">{student.name.split(" ")[0]}</h1>
            </div>
          </div>
          <div className="flex gap-3">
            <a href="https://wa.me/2348106293674" target="_blank" rel="noopener noreferrer" className="bg-green-500 p-3.5 rounded-2xl border border-white/20 text-white hover:bg-green-400 transition-all shadow-lg animate-pulse"><Headset size={20} /></a>
            <button onClick={triggerLogout} className="bg-[#006400] p-3.5 rounded-2xl border border-white/10 hover:bg-red-600 transition-colors shadow-lg"><LogOut size={20} /></button>
          </div>
        </div>
        <div className="bg-[#003300]/50 backdrop-blur-md border border-white/10 rounded-3xl p-6 flex items-center justify-between">
          ...
          <div className="bg-white text-[#004d00] px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wide shadow-sm">Active</div>
        </div>
      </header>
      ...
      <div className="px-6 -mt-10 relative z-20 space-y-8">
        ...
        <section>
          ...
          {examHistory.length > 0 ? (
            ...
            <div className="bg-white rounded-3xl p-10 border border-gray-100 shadow-sm text-center"><p className="text-[10px] font-black text-gray-300 uppercase tracking-widest">No missions completed yet</p></div>
          )}
        </section>
        ...
        <section>
          ...
          {leaders.length > 0 ? (
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 overflow-x-auto flex gap-5 custom-scrollbar">
              {leaders.map((user, i) => (
            ...
            <div className="text-center py-12 bg-white rounded-3xl border border-gray-100"><p className="text-gray-400 text-[10px] font-black uppercase tracking-widest">Awaiting Rankings...</p></div>
          )}
        </section>
      </div>
