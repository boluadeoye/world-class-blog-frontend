...
/* === 1. EXAM SETUP MODAL (RESTORED & SECURED) === */
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-sm overflow-hidden border border-white animate-in zoom-in duration-200">
        <div className="bg-[#004d00] p-8 text-white relative">
          ...
          <h3 className="font-black text-sm uppercase tracking-[0.2em] flex items-center gap-2">
            <Settings size={18} /> {isBlocked ? "Access Denied" : "Configure Session"}
          </h3>
          ...
          {isBlocked ? (
            ...
            <>
              <div className="mb-8">
                  ...
                  {[15, 30, 45, 60].map((time) => (
                    <button
                      ...
                      className={`py-4 rounded-2xl text-xs font-black transition-all relative overflow-hidden border-2 ${
                        duration === time ? 'border-green-600 bg-green-50 text-green-900 shadow-inner' : 'border-gray-100 text-gray-400 bg-gray-50'
                      }`}
                    >
                      {time}m
                ...
                {!isPremium && (
                  <button onClick={onUpgrade} className="mt-4 w-full flex items-center justify-center gap-2 text-[9px] text-yellow-700 bg-yellow-50 p-3 rounded-xl border border-yellow-100">
              ...
              <div className="flex gap-3">
                <button onClick={onClose} className="flex-1 py-4 border-2 border-gray-100 rounded-2xl text-[10px] font-black text-gray-400 uppercase tracking-widest">Cancel</button>
...
function CourseCard({ course, onLaunch, variant = "green", isPremium }) {
  ...
  return (
    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex justify-between items-center group active:scale-[0.98] transition-transform mb-3">
      ...
      <button onClick={() => onLaunch(course)} className={`w-9 h-9 rounded-full flex items-center justify-center shadow-lg transition-colors ${isBlocked ? 'bg-red-50 text-red-400' : isGst ? 'bg-gray-900 text-white hover:bg-[#004d00]' : 'bg-blue-900 text-white hover:bg-blue-950'}`}>
...
export default function StudentDashboard() {
  ...
  return (
    <main className="min-h-screen bg-[#fcfdfc] font-sans text-gray-900 pb-48 relative">
      ...
      <div className="px-6 -mt-8 relative z-20 space-y-6">
        ...
        <section className="bg-[#f0f4ff] rounded-[2.5rem] shadow-xl shadow-blue-900/5 border border-blue-100 p-6">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3"><div className="bg-blue-600 p-2 rounded-xl text-white shadow-lg shadow-blue-200"><Layers size={18} /></div><h2 className="font-black text-sm text-blue-900 uppercase tracking-widest">Other Courses</h2></div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-4 overflow-x-auto flex gap-4 custom-scrollbar">
              {leaders.map((user, i) => (
                <div key={i} className="min-w-[180px] bg-gray-50 rounded-2xl p-5 border border-gray-100 flex flex-col items-center text-center relative">
                  ...
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-sm mb-3 overflow-hidden border-2 ${i === 0 ? 'border-yellow-400' : 'border-gray-100'}`}><img src={`https://api.dicebear.com/9.x/notionists/svg?seed=${user.name.replace(/\s/g, '')}&backgroundColor=transparent`} alt={user.name} className="w-full h-full object-cover" /></div>
      ...
      <div className="fixed bottom-6 left-6 right-6 z-50"><div className="bg-white/90 backdrop-blur-xl border border-white/40 shadow-[0_10px_40px_rgba(0,0,0,0.1)] rounded-[2rem] p-4 flex items-center gap-4"><div className="w-10 h-10 bg-[#004d00] rounded-full flex items-center justify-center text-white shadow-md shrink-0"><Award size={18} /></div><div className="flex-1 min-w-0"><p className="text-[8px] font-black text-gray-400 uppercase tracking-[0.2em] mb-0.5">Engineered By</p><h4 className="font-black text-xs text-gray-900 truncate">BOLU ADEOYE</h4><p className="text-[9px] text-[#004d00] font-bold truncate opacity-80">In conjunction with Abel Kings Educational Center</p></div><div className="h-8 w-[1px] bg-gray-200 mx-1"></div><div className="text-right shrink-0"><p className="text-[8px] font-black text-gray-300 uppercase tracking-widest">FUOYE</p><p className="text-[10px] font-black text-gray-900">2026</p></div></div></div>
    </main>
  );
}
