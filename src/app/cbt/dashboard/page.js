...
function CourseCard({ course, onLaunch, variant = "green", isPremium }) {
  ...
  const isBlocked = !isPremium && course.user_attempts >= 2;
  return (
    <div className="bg-white p-3 rounded-2xl shadow-[0_2px_8px_-2px_rgba(0,0,0,0.05)] border border-gray-50 flex justify-between items-center group active:scale-[0.99] transition-transform mb-3">
      <div className="flex items-center gap-4 flex-1 min-w-0">
        <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-[10px] border shrink-0 ${isGst ? 'bg-green-50 text-[#004d00] border-green-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{course.code.slice(0,3)}</div>
        <div className="min-w-0 flex-1">
          <h3 className="font-black text-gray-900 text-xs uppercase tracking-tight truncate">{course.code}</h3>
          <p className="text-[9px] text-gray-400 font-medium truncate">{course.title}</p>
        </div>
      </div>
      <button onClick={(e) => { e.preventDefault(); onLaunch(course); }} className={`w-8 h-8 rounded-full flex items-center justify-center shadow-md transition-all shrink-0 ml-3 ${isBlocked ? 'bg-red-50 text-red-400 border border-red-100' : isGst ? 'bg-gray-900 text-white hover:bg-[#004d00]' : 'bg-blue-900 text-white hover:bg-blue-950'}`}>{isBlocked ? <Lock size={12} /> : <Play size={12} fill="currentColor" />}</button>
    </div>
  );
}
