...
function DisclaimerCard() {
  ...
  return (
    <div className="bg-[#FFF8F0] border border-orange-100 rounded-[2.5rem] overflow-hidden mb-8 shadow-sm">
      ...
      {isOpen && (
        <div className="px-8 pb-10 text-xs text-[#8B5E3C] leading-relaxed border-t border-orange-50 pt-4">
...
function CourseCard({ course, onLaunch, variant = "green", isPremium }) {
  ...
  return (
    <div className="bg-white p-5 rounded-[2rem] shadow-sm border border-gray-100 flex justify-between items-center group active:scale-[0.98] transition-transform mb-4">
      <div className="flex items-center gap-5">
        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-sm border ${isGst ? 'bg-green-50 text-[#004d00] border-green-100' : 'bg-blue-50 text-blue-700 border-blue-100'}`}>{course.code.slice(0,3)}</div>
        <div><h3 className="font-black text-gray-900 text-sm uppercase tracking-tight">{course.code}</h3><p className="text-[10px] text-gray-500 font-medium truncate w-40">{course.title}</p></div>
      </div>
      <button onClick={(e) => { e.preventDefault(); onLaunch(course); }} className={`w-11 h-11 rounded-full flex items-center justify-center shadow-lg transition-all ${isBlocked ? 'bg-red-50 text-red-400 border border-red-100' : isGst ? 'bg-gray-900 text-white hover:bg-[#004d00]' : 'bg-blue-900 text-white hover:bg-blue-950'}`}>{isBlocked ? <Lock size={16} /> : <Play size={16} fill="currentColor" />}</button>
    </div>
  );
}
