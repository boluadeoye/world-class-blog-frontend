...
export default function ExamPage() {
  ...
  // 1. IRON MODE SECURITY (Anti-Copy & Focus)
  useEffect(() => {
    ...
    async function loadExam() {
      try {
        const query = new URLSearchParams({
          ...
          token: parsedStudent.session_token || "" // THE HANDSHAKE
        });

        const res = await fetch(`/api/cbt/exam?${query.toString()}`);
        const data = await res.json();

        // THE KILL SWITCH: Token Mismatch
        if (res.status === 401) { 
          sessionStorage.removeItem("cbt_student"); // Nuke session
          alert("SESSION TERMINATED: Logged in on another device.");
          router.push("/cbt"); 
          return; 
        }
        
        // THE FREEMIUM GATE: Limit Reached
        if (res.status === 403) { 
          setShowUpgrade(true); 
          setLoading(false); 
          return; 
            ...
            <div className="space-y-4">
              {questions.map((q, i) => (
                <div key={q.id} className={`p-6 border-l-4 bg-white shadow-sm ${answers[q.id] === q.correct_option ? 'border-green-500' : 'border-red-500'}`}>
                  ...
                  {/* FREEMIUM GATE: EXPLANATIONS */}
                  {q.explanation && (
                    <div className={`mt-4 p-4 text-sm border-l-4 ${isPremium ? 'bg-blue-50 text-blue-900 border-blue-500' : 'bg-gray-100 text-gray-400 border-gray-300'}`}>
                      <span className="font-black uppercase text-xs block mb-1">Intel</span>
                      {isPremium ? q.explanation : <span className="blur-sm select-none">This explanation is available to Premium Users only. Upgrade to unlock detailed analysis.</span>}
                    </div>
                  )}
                </div>
              ))}
