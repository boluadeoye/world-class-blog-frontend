...
import { Grid, CheckCircle, AlertOctagon, X, Crown, Sparkles, BrainCircuit, Clock, ChevronRight, ChevronLeft } from "lucide-react";
...
function TimeUpOverlay() {
  return (
    <div className="fixed inset-0 z-[600] bg-black flex flex-col items-center justify-center text-white p-6">
      <Clock size={48} className="text-red-500 animate-spin mb-4" />
      <h2 className="text-3xl font-black uppercase tracking-widest">Time Expired</h2>
      <p className="text-emerald-500 font-bold text-xs animate-pulse mt-2">Securing Responses...</p>
    </div>
  );
}
    ...
    <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/90 backdrop-blur-md p-6">
      <div className="bg-white rounded-[2rem] shadow-2xl max-w-sm w-full overflow-hidden">
        <div className="bg-green-50 p-8 text-center border-b border-green-100">
          ...
          <h3 className="font-black text-xl text-green-900 uppercase">Final Audit</h3>
          <p className="text-sm font-bold text-green-700 mt-2">{answeredCount} / {totalCount} Answered</p>
        </div>
        <div className="p-6 flex gap-4">
          <button onClick={onCancel} className="flex-1 py-4 border-2 border-gray-100 rounded-2xl text-xs font-black text-gray-400 uppercase">Review</button>
          <button onClick={onConfirm} className="flex-[1.5] py-4 bg-[#004d00] text-white rounded-2xl text-xs font-black shadow-lg uppercase">Submit</button>
        </div>
      </div>
    </div>
  );
...
function ExamContent() {
  ...
  useEffect(() => {
    ...
    async function loadExam() {
      try {
        const query = new URLSearchParams({ courseId: params.id, studentId: parsed.id, token: parsed.session_token || "" });
        ...
        if (saved) {
          const s = JSON.parse(saved); setAnswers(s.answers || {}); setTimeLeft(s.timeLeft); setCurrentQIndex(s.currentIndex || 0);
        } else {
          const reqDur = searchParams.get('duration');
          setTimeLeft(((data.isPremium && reqDur) ? parseInt(reqDur) : (data.course?.duration || 15)) * 60);
        }
  ...
  const submitExam = useCallback(async () => {
    ...
    setScore(correct); setIsSubmitted(true); setIsTimeUp(false); setShowSubmitModal(false);
    window.scrollTo({ top: 0, behavior: 'smooth' });
    if (student && course) {
      try { await fetch('/api/cbt/result', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId: student.id, courseId: course.id, score: correct, total: questions.length, answers }) }); } catch(e) {}
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 0) { clearInterval(timer); setIsTimeUp(true); setTimeout(submitExam, 3000); return 0; }
        if (student && prev % 5 === 0) localStorage.setItem(getStorageKey(student.email), JSON.stringify({ answers, timeLeft: prev, currentIndex: currentQIndex }));
        return prev - 1;
      });
    }, 1000);
  ...
  const handleSelect = (opt) => { if (!isSubmitted && !isTimeUp) setAnswers(prev => ({ ...prev, [questions[currentQIndex].id]: opt })); };
  const navigateTo = (i) => { setCurrentQIndex(i); setShowMap(false); };

  const generateAnalysis = async () => {
    setAnalyzing(true);
    const failed = questions.filter(q => answers[q.id] !== q.correct_option).map(q => ({ question_text: q.question_text, correct_option: q.correct_option, user_choice: answers[q.id] || "Skipped" }));
    try {
      const res = await fetch("/api/cbt/analyze", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ studentName: student.name, courseCode: course.code, score, total: questions.length, failedQuestions: failed }) });
      const data = await res.json(); setAnalysis(String(data.analysis));
    } catch (e) { alert("AI Error."); } finally { setAnalyzing(false); }
  };

  const getGridColor = (i, qId) => {
    if (currentQIndex === i) return "bg-yellow-400 text-black border-yellow-500 ring-2 ring-yellow-100";
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-white text-green-900 font-black text-sm tracking-widest animate-pulse">SYNCING TERMINAL...</div>;
  if (error) return <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 p-6 text-center text-red-600 font-bold gap-4"><p>{error}</p><button onClick={() => window.location.reload()} className="bg-black text-white px-8 py-3 rounded-xl text-xs font-black">RETRY</button></div>;
      ...
      <main className="min-h-screen bg-[#f8fafc] font-sans pb-20 overflow-y-auto">
        <header className="bg-[#002b00] text-white pt-10 pb-20 px-6 rounded-b-[3rem] shadow-2xl relative overflow-hidden">
          <div className="relative z-10 flex justify-between items-start mb-8">
            ...
            <button onClick={() => router.push('/cbt/dashboard')} className="bg-white/10 border border-white/10 px-5 py-2 rounded-full text-[10px] font-bold uppercase">Exit</button>
          </div>
          <div className="relative z-10 flex flex-col items-center">
            <div className={`text-7xl font-black tracking-tighter ${pct >= 50 ? 'text-white' : 'text-red-400'}`}>{pct}%</div>
            <div className="flex gap-4 mt-4 text-[10px] font-black uppercase tracking-widest text-gray-400"><span>Score: {score}/{questions.length}</span><span>•</span><span>{pct >= 50 ? 'PASSED' : 'FAILED'}</span></div>
          </div>
        </header>
        <div className="max-w-3xl mx-auto px-6 mt-6 relative z-20">
          <div className="bg-white rounded-2xl shadow-lg p-1.5 mb-6 flex gap-2 border border-gray-100">
            ...
            <button onClick={() => setActiveTab("ai")} className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'ai' ? 'bg-purple-900 text-white shadow-md' : 'text-gray-400'}`}><Sparkles size={14} /> Study Room</button>
          </div>
          {activeTab === "corrections" ? (
            <div className="space-y-4">
              {questions.map((q, i) => (
                <div key={q.id} className={`bg-white p-6 rounded-3xl shadow-sm border-l-4 ${answers[q.id] === q.correct_option ? 'border-green-500' : 'border-red-500'}`}>
                  <div className="flex justify-between items-start mb-4"><span className="bg-gray-100 text-gray-500 px-3 py-1 rounded-lg text-[10px] font-black uppercase">Q{i+1}</span>{answers[q.id] === q.correct_option ? <CheckCircle size={20} className="text-green-600" /> : <X size={20} className="text-red-500" />}</div>
                  <p className="font-bold text-gray-900 text-sm leading-relaxed mb-4">{q.question_text}</p>
                  <div className="grid grid-cols-2 gap-3 mb-4">
                    <div className="bg-green-50 p-3 rounded-xl"><p className="text-[10px] font-black text-green-800 uppercase mb-1">Correct</p><p className="text-sm font-bold text-green-900">{q.correct_option}</p></div>
                    {answers[q.id] !== q.correct_option && <div className="bg-red-50 p-3 rounded-xl"><p className="text-[10px] font-black text-red-800 uppercase mb-1">Yours</p><p className="text-sm font-bold text-red-900">{answers[q.id] || "Skipped"}</p></div>}
                  </div>
                  {isPremium && q.explanation && <div className="mt-4 pt-4 border-t border-gray-100"><p className="text-[10px] font-black text-blue-600 uppercase tracking-widest mb-1">Intel</p><p className="text-xs text-gray-600 leading-relaxed italic">{q.explanation}</p></div>}
                </div>
              ))}
            </div>
          ) : (
            <div className="bg-white rounded-[2.5rem] shadow-xl border border-gray-100 overflow-hidden min-h-[500px] relative">
              {!isPremium ? (
                <div className="absolute inset-0 z-10 bg-white/95 flex flex-col items-center justify-center text-center p-10">
                  <div className="w-24 h-24 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-[2rem] flex items-center justify-center mb-8 shadow-xl animate-bounce"><Crown size={48} className="text-white" /></div>
                  <h3 className="text-2xl font-black text-gray-900 mb-3 uppercase tracking-tighter">Restricted Intel</h3>
                  <button onClick={() => setShowUpgrade(true)} className="bg-gray-900 text-white px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg">Unlock Study Plan</button>
                </div>
              ) : (
                <div className="p-0">
                  {!analysis ? (
                    <div className="text-center py-32 px-8 bg-white">
                      <BrainCircuit size={40} className="text-purple-600 mx-auto mb-6 animate-pulse" />
                      <h3 className="font-black text-gray-900 text-xs uppercase tracking-widest mb-2">Analyzing Performance</h3>
                      <button onClick={generateAnalysis} disabled={analyzing} className="bg-purple-900 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl">{analyzing ? "PROCESSING..." : "GENERATE REPORT"}</button>
                    </div>
                  ) : (
                    <div className="bg-[#fcfcfc] min-h-[600px]">
                      <div className="bg-purple-900 text-white p-10 pb-16 rounded-b-[3.5rem] shadow-lg">
                      ...
                      <div className="px-6 mt-6 pb-20">
                        <div className="bg-white rounded-[2.5rem] shadow-xl border border-purple-100 p-8 text-left border-l-[8px] border-l-purple-600">
  ...
  if (!currentQ) return <div className="h-screen flex items-center justify-center bg-white font-black text-xs tracking-widest text-green-900">SYNCING...</div>;
    ...
    <main className="h-screen flex flex-col bg-[#f0f2f5] font-sans overflow-hidden select-none">
      ...
      <header className="h-14 bg-[#004d00] text-white flex justify-between items-center px-4 shrink-0 z-[160] border-b border-green-800">
        <div className="flex items-center gap-3">
          ...
          <div className="leading-tight"><h1 className="font-black text-[10px] uppercase tracking-widest text-green-100 truncate w-24">{student?.name}</h1><p className="text-[9px] font-mono opacity-70 uppercase">{course?.code}</p></div>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 bg-black/30 px-3 py-1 rounded-full border border-white/10"><Clock size={12} className={timeLeft < 300 ? "text-red-500" : "text-green-400"} /><span className={`font-mono font-black text-sm tracking-widest ${timeLeft < 300 ? "text-red-500" : "text-white"}`}>{Math.floor(timeLeft/60)}:{String(timeLeft%60).padStart(2,'0')}</span></div>
