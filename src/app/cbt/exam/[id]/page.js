  ...
  <div className="fixed inset-0 z-[50] pointer-events-none overflow-hidden flex items-center justify-center opacity-[0.03]">
    <div className="absolute inset-0 flex flex-wrap content-center justify-center gap-20 transform -rotate-12 scale-150">
      {Array.from({ length: 20 }).map((_, i) => (
        <div key={i} className="text-4xl font-black uppercase tracking-widest text-gray-900 whitespace-nowrap">{text} • OFFICIAL USE ONLY</div>
      ))}
    </div>
  </div>
);
function ExamContent() {
  ...
  useEffect(() => {
    ...
    async function loadExam() {
      ...
      try {
        const res = await fetch(`/api/cbt/exam?courseId=${params.id}&studentId=${parsed.id}&token=${parsed.session_token}&deviceId=${hwId}&limit=${limit}`);
        ...
        if (saved) {
          ...
          // FIX: If saved time is drastically different from requested time, wipe it.
          if (Math.abs(s.timeLeft - (finalDur * 60)) > 300 && s.timeLeft < (finalDur * 60)) {
            localStorage.removeItem(getStorageKey(parsed.email));
            setTimeLeft(finalDur * 60);
          } else {
            setAnswers(s.answers || {}); setTimeLeft(s.timeLeft); setCurrentQIndex(s.currentIndex || 0);
          }
  ...
  const submitExam = async () => {
    ...
    if (student && course) {
      await fetch('/api/cbt/result', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId: student.id, courseId: course.id, score: correct, total: questions.length, answers }) });
  ...
  }, [loading, isSubmitted, timeLeft, answers, currentQIndex, student]);
  if (loading) return <div className="h-screen flex flex-col items-center justify-center bg-[#002b00] text-white"><Fingerprint size={40} className="animate-pulse text-green-400 mb-4" /><h2 className="font-black text-xl uppercase tracking-[0.3em]">Authenticating</h2></div>;
    ...
    return (
      <main className="min-h-screen bg-[#f0f2f5] pb-20">
        <header className="bg-[#002b00] text-white pt-10 pb-20 px-6 rounded-b-[3rem] text-center relative overflow-hidden">
          <div className="relative z-10"><h1 className="text-4xl font-black mb-2">{pct}%</h1><p className="text-[10px] font-black uppercase tracking-widest opacity-60">Score: {score}/{questions.length}</p><button onClick={() => router.push('/cbt/dashboard')} className="mt-6 bg-white/10 border border-white/20 px-6 py-2 rounded-full text-[10px] font-bold uppercase">Exit</button></div>
        </header>
        <div className="max-w-3xl mx-auto px-4 -mt-10 relative z-20">
          <div className="bg-white rounded-2xl shadow-lg p-1.5 mb-6 flex gap-2">
            ...
            <button className="flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase text-gray-400">Intelligence</button>
          </div>
            ...
            <div className="space-y-4">
              {questions.map((q, i) => (
                <div key={q.id} className={`bg-white p-6 rounded-3xl shadow-sm border-l-[6px] ${answers[q.id] === q.correct_option ? 'border-green-500' : 'border-red-500'}`}>
                  ...
                  <div className="bg-green-50 p-4 rounded-xl border border-green-100 mb-2"><p className="text-sm font-bold text-green-900">{q.correct_option}. {q[`option_${q.correct_option.toLowerCase().trim()}`]}</p></div>
                  {answers[q.id] !== q.correct_option && <div className="bg-red-50 p-4 rounded-xl border border-red-100"><p className="text-sm font-bold text-red-900">{answers[q.id] ? `${answers[q.id]}. ${q[`option_${answers[q.id].toLowerCase().trim()}`]}` : "Skipped"}</p></div>}
                </div>
              ))}
            </div>
          )}
      ...
      <div className="flex-1 p-4 overflow-hidden flex flex-col">
        <div className="flex-1 bg-white rounded-[2.5rem] shadow-xl p-6 flex flex-col justify-between overflow-hidden border border-gray-100">
          ...
          <div className="grid gap-3">
            {['A','B','C','D'].map(opt => (
              <button key={opt} onClick={() => setAnswers({...answers, [questions[currentQIndex].id]: opt})} className={`p-4 rounded-2xl border-2 text-left flex items-center gap-4 transition-all ${answers[questions[currentQIndex].id] === opt ? 'border-green-600 bg-green-50' : 'border-gray-100 bg-white'}`}>
