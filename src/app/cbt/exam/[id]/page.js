...
/* === INTERNAL COMPONENT: LIVE TRACKER (HARDWARE LOCK) === */
...
/* === INTERNAL COMPONENT: UPGRADE MODAL (PAYMENT LOGIC) === */
function InternalUpgradeModal({ student, onClose, onSuccess }) {
  ...
  const handleVerify = async () => {
    ...
    setLoading(true);
    try {
      // FIX: Pointing to the CORRECT API path found in your backend
      ...
      if (data.success) { 
        alert("Success! Premium Access Granted."); 
        onSuccess(); 
      } else { 
        alert("Failed: " + (data.error || "Verification failed")); 
      }
    } catch (e) { 
      alert("System Error: Could not verify payment."); 
    }
    setLoading(false);
  };
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/90 p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-sm rounded-2xl p-6 text-center shadow-2xl">
        ...
        <h2 className="text-xl font-bold mb-2 text-gray-900">Premium Access</h2>
        <p className="text-sm text-gray-500 mb-6">Enter your payment reference to unlock full access.</p>
        
        <button 
          onClick={handleVerify} 
          disabled={loading}
          className="bg-green-700 text-white px-6 py-3.5 rounded-xl w-full font-bold text-sm uppercase tracking-widest hover:bg-green-800 transition-colors mb-3"
        >
          {loading ? "Verifying..." : "I Have Paid"}
        </button>
        
        <button onClick={onClose} className="text-gray-400 text-xs font-bold uppercase tracking-widest hover:text-gray-600">
          Close
        </button>
      </div>
    </div>
...
function ExamContent() {
  ...
  // Refs for state access inside intervals without dependencies
        ...
        try {
            await fetch('/api/cbt/result', {
                ...
                body: JSON.stringify({ 
                  ...
                  answers: answers,
                  deviceId: hwId // CRITICAL: Send Hardware ID
                })
            });
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        // Use refs to access latest state without resetting interval
        if (studentRef.current && newTime % 5 === 0) {
          localStorage.setItem(
            getStorageKey(studentRef.current.email), 
            JSON.stringify({ answers: answersRef.current, timeLeft: newTime, currentIndex: currentQIndex }) 
          );
        }
        return newTime;
      });
  ...
  // === INLINE UPGRADE MODAL RENDER ===
  if (showUpgrade) return (
    <InternalUpgradeModal 
      student={student} 
