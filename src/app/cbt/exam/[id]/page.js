...
import {
  ...
  BrainCircuit, Clock, ChevronRight, ChevronLeft,
  Loader2, BookOpen, Zap, Lock, Fingerprint, XCircle, AlertTriangle, LogOut
} from "lucide-react";
...
function InternalStatusModal({ type, title, message, actionLabel, onAction, onCancel }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-sm p-6 animate-in fade-in duration-200">
      <div className="bg-white rounded-[2rem] shadow-2xl max-w-sm w-full overflow-hidden border border-gray-100">
        ...
        <div className="p-6 bg-white flex gap-3">
          {onCancel && <button onClick={onCancel} className="flex-1 py-4 border-2 border-gray-100 rounded-2xl text-xs font-black text-gray-400 hover:bg-gray-50 uppercase tracking-widest transition-all">Cancel</button>}
          <button onClick={onAction} className={`flex-[1.5] py-4 ${config.btn} text-white rounded-2xl text-xs font-black shadow-lg transition-all active:scale-95 uppercase tracking-widest`}>{actionLabel || "Acknowledge"}</button>
        </div>
      </div>
    </div>
  );
...
function InternalUpgradeModal({ student, onClose, onSuccess }) {
      ...
      callback: async function(response) {
        ...
        try {
          const res = await fetch("/api/cbt/payment/verify", {
            ...
            body: JSON.stringify({ reference: response.reference, studentId: student.id }),
          });
          if (res.ok) { setStatus({ type: 'success', title: 'Access Granted', message: 'Premium features are now active.' }); }
  ...
  return (
    <>
      {status && <InternalStatusModal {...status} onAction={() => { if(status.type === 'success') onSuccess(); setStatus(null); }} />}
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-300">
        <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-sm w-full overflow-hidden relative border border-white animate-in zoom-in duration-300">
          <button onClick={onClose} className="absolute top-6 right-6 p-2 bg-black/10 hover:bg-black/20 text-black rounded-full transition-colors z-20"><X size={18} strokeWidth={3} /></button>
          <div className="bg-gradient-to-br from-yellow-400 via-orange-500 to-yellow-600 p-10 text-center text-white relative">
            ...
            <div className="relative z-10">
              <div className="mx-auto w-20 h-20 bg-white/20 rounded-3xl flex items-center justify-center mb-4 backdrop-blur-md border border-white/30 shadow-xl animate-bounce"><Crown size={40} fill="currentColor" /></div>
          ...
          <div className="p-8 bg-white">
            <div className="space-y-4 mb-8">
              {benefits.map((b, i) => (<div key={i} className="flex items-center gap-4 text-gray-700 group"><div className="w-8 h-8 rounded-lg bg-green-50 flex items-center justify-center text-green-600 group-hover:bg-green-600 group-hover:text-white transition-colors">{b.icon}</div><span className="text-xs font-black uppercase tracking-widest">{b.text}</span></div>))}
            </div>
            {verifying ? (<div className="w-full py-4 bg-gray-900 text-white rounded-2xl font-black flex items-center justify-center gap-2 animate-pulse"><Loader2 size={20} className="animate-spin" /> VERIFYING...</div>) : (<button onClick={handlePayment} disabled={!scriptLoaded} className="w-full py-5 bg-gray-900 hover:bg-black text-white rounded-2xl font-black shadow-xl transform active:scale-95 transition-all uppercase tracking-[0.2em] text-xs disabled:opacity-50">PAY ₦500 NOW</button>)}
...
function InternalLiveTracker() {
  const pathname = usePathname();
  useEffect(() => {
    const generateFingerprint = () => {
      try {
        const signals = [navigator.userAgent, navigator.language, screen.colorDepth, screen.width + 'x' + screen.height, new Date().getTimezoneOffset(), navigator.hardwareConcurrency || 'u'].join('||');
        let hash = 0;
        for (let i = 0; i < signals.length; i++) { const char = signals.charCodeAt(i); hash = ((hash << 5) - hash) + char; hash = hash & hash; }
    ...
    const initTracker = async () => {
      ...
      const sendPulse = async () => {
        ...
        try { await fetch("/api/cbt/heartbeat", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ studentId: student.id, name: student.name, page: pathname, action: pathname.includes("exam") ? "Taking Exam" : "Browsing Dashboard", deviceId: stickyId }) }); } catch (e) { }
      };
      sendPulse();
  ...
  return (
    <div className="fixed inset-0 z-[50] pointer-events-none overflow-hidden flex items-center justify-center opacity-[0.03]">
      <div className="absolute inset-0 flex flex-wrap content-center justify-center gap-20 transform -rotate-12 scale-150">
        {Array.from({ length: 20 }).map((_, i) => (<div key={i} className="text-4xl font-black uppercase tracking-widest text-gray-900 whitespace-nowrap select-none">{text} • OFFICIAL USE ONLY • {text}</div>))}
      </div>
    </div>
  );
}
...
function TimeUpOverlay() {
  return (
    <div className="fixed inset-0 z-[600] bg-[#050505] flex flex-col items-center justify-center text-white p-6 animate-in fade-in duration-500">
      <div className="w-24 h-24 bg-red-600/10 border-2 border-red-600 rounded-full flex items-center justify-center mb-6"><Clock size={48} className="text-red-500 animate-spin-slow" /></div>
      ...
      <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-sm w-full overflow-hidden border border-green-100">
        <div className="bg-green-50 p-8 flex flex-col items-center text-center border-b border-green-100">
          ...
          <div className="mt-4 w-full space-y-2">
            <div className="flex justify-between items-center bg-white/60 p-3 rounded-xl border border-green-100"><span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Answered</span><span className="text-sm font-black text-green-700">{answeredCount} / {totalCount}</span></div>
            {pendingCount > 0 && (<div className="flex justify-between items-center bg-red-50 p-3 rounded-xl border border-red-100"><span className="text-[10px] font-black text-red-400 uppercase tracking-widest">Pending</span><span className="text-sm font-black text-red-600">{pendingCount} Left</span></div>)}
          </div>
        </div>
...
function ExamContent() {
  ...
  useEffect(() => {
    ...
    async function loadExam() {
      try {
        ...
        const query = new URLSearchParams({ courseId: params.id, studentId: parsedStudent.id, token: parsedStudent.session_token || "", deviceId: hwId, limit: limit });
        const res = await fetch(`/api/cbt/exam?${query.toString()}`);
        if (res.status === 403) { setLimitReached(true); setLoading(false); return; }
        if (!res.ok) { if (res.status === 401) { router.push("/cbt"); return; } throw new Error("Data retrieval failed."); }
        ...
        if (savedSession) {
          ...
          if (Math.abs(session.timeLeft - (finalDur * 60)) > 300 && session.timeLeft < (finalDur * 60)) { localStorage.removeItem(getStorageKey(parsedStudent.email)); setTimeLeft(finalDur * 60); }
          else { setAnswers(session.answers || {}); setTimeLeft(session.timeLeft); setCurrentQIndex(session.currentIndex || 0); }
        } else { setTimeLeft(finalDur * 60); }
  ...
  const submitExam = useCallback(async () => {
    ...
    const hwId = localStorage.getItem("cbt_hw_id") || "unknown";
    if (student && course) {
        try { await fetch('/api/cbt/result', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId: student.id, courseId: course.id, score: correctCount, total: questions.length, answers: answers, deviceId: hwId }) }); } catch(e) { }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setTimeLeft((prev) => {
        ...
        if (studentRef.current && newTime % 5 === 0) { localStorage.setItem(getStorageKey(studentRef.current.email), JSON.stringify({ answers: answersRef.current, timeLeft: newTime, currentIndex: currentQIndex })); }
        return newTime;
      });
    }, 1000);
    return () => clearInterval(interval);
  ...
  const handleSelect = (option) => { if (!isSubmitted && !isTimeUp && questions[currentQIndex]) { setAnswers(prev => ({ ...prev, [questions[currentQIndex].id]: option })); } };
  ...
  if (limitReached) return (
    <div className="min-h-screen bg-[#002b00] flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <div className="relative z-10 bg-white rounded-[2.5rem] max-w-sm w-full text-center shadow-2xl overflow-hidden">
         <div className="bg-[#004d00] p-8 relative"><div className="w-20 h-20 bg-white rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg"><Lock size={40} className="text-[#004d00]" /></div><h2 className="text-2xl font-black text-white uppercase tracking-widest">Access Denied</h2><p className="text-green-200 text-[10px] font-mono mt-2 uppercase tracking-[0.2em]">Hardware Limit Reached (2/2)</p></div>
         <div className="p-8"><p className="text-gray-600 text-xs font-medium leading-relaxed mb-8">Your device has exhausted the free attempt allocation for this sector.</p><button onClick={() => setShowUpgrade(true)} className="w-full py-4 bg-[#004d00] text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl hover:bg-green-900 transition-all active:scale-95 flex items-center justify-center gap-2"><Crown size={14} /> Upgrade Clearance</button><button onClick={() => router.push('/cbt/dashboard')} className="mt-6 text-gray-400 text-[10px] font-black uppercase tracking-widest hover:text-gray-600">Return to Base</button></div>
      </div>
    </div>
  );
  if (showUpgrade) return <InternalUpgradeModal student={student} onClose={() => setShowUpgrade(false)} onSuccess={() => window.location.reload()} />;
    ...
    return (
      <main className="min-h-screen bg-[#f0f2f5] font-sans pb-20 overflow-y-auto">
        <header className="bg-[#002b00] text-white pt-10 pb-20 px-6 rounded-b-[3rem] shadow-2xl relative overflow-hidden">
          ...
          <div className="relative z-10 flex justify-between items-start mb-8"><div><div className="text-[9px] font-black text-green-400 uppercase tracking-widest mb-1">Session Closed</div><h1 className="font-black text-2xl tracking-tight">{course?.code}</h1></div><button onClick={() => router.push('/cbt/dashboard')} className="bg-white/10 backdrop-blur-md border border-white/10 px-5 py-2 rounded-full text-[10px] font-bold uppercase hover:bg-white hover:text-[#002b00] transition-colors">Exit</button></div>
          <div className="relative z-10 flex flex-col items-center"><div className="w-32 h-32 flex items-center justify-center relative"><svg className="absolute w-full h-full" viewBox="0 0 36 36"><path className="text-white/10" strokeDasharray="100, 100" d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="2.5" /><path className={percentage >= 50 ? "text-emerald-500" : "text-red-500"} strokeDasharray={`${percentage}, 100`} d="M18 2.0845 a 15.9155 15.9155 0 0 1 0 31.831 a 15.9155 15.9155 0 0 1 0 -31.831" fill="none" stroke="currentColor" strokeWidth="2.5" /></svg><div className="text-4xl font-black">{percentage}%</div></div><div className="mt-6 flex gap-4 text-[10px] font-black uppercase tracking-widest text-gray-400"><span>Score: {score}/{questions.length}</span><span className="w-1 h-1 bg-gray-600 rounded-full mt-1.5"></span><span>Answered: {answeredCount}</span></div></div>
        </header>
        <div className="max-w-3xl mx-auto px-4 -mt-10 relative z-20">
          <div className="bg-white rounded-2xl shadow-lg p-1.5 mb-6 flex gap-2 border border-gray-100"><button onClick={() => setActiveTab("corrections")} className={`flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'corrections' ? 'bg-[#004d00] text-white shadow-md' : 'text-gray-400'}`}>Corrections</button><button onClick={() => setActiveTab("ai")} className={`flex-1 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 ${activeTab === 'ai' ? 'bg-purple-900 text-white shadow-md' : 'text-gray-400'}`}><Sparkles size={14} /> Intelligence</button></div>
          {activeTab === "corrections" ? (<div className="space-y-4">{questions.map((q, i) => (<div key={q.id} className={`bg-white p-6 rounded-3xl shadow-sm border-l-[6px] ${answers[q.id] === q.correct_option ? 'border-green-500' : 'border-red-500'}`}><div className="flex justify-between items-start mb-4"><span className="bg-gray-100 text-gray-500 px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest">Question {i+1}</span>{answers[q.id] === q.correct_option ? <CheckCircle size={20} className="text-green-600" /> : <X size={20} className="text-red-500" />}</div><p className="font-bold text-gray-900 text-sm leading-relaxed mb-6">{q.question_text}</p><div className="grid grid-cols-1 gap-3 mb-4"><div className="bg-green-50 border border-green-100 p-4 rounded-xl"><p className="text-[9px] font-black text-green-800 uppercase mb-1 flex items-center gap-2"><CheckCircle size={12}/> Correct Answer</p><p className="text-sm font-bold text-green-900 leading-snug"><span className="font-black mr-2">{q.correct_option}.</span>{q[`option_${q.correct_option.toLowerCase()}`] || "Option text unavailable"}</p></div>{answers[q.id] !== q.correct_option && (<div className="bg-red-50 border border-red-100 p-4 rounded-xl"><p className="text-[9px] font-black text-red-800 uppercase mb-1 flex items-center gap-2"><X size={12}/> Your Selection</p><p className="text-sm font-bold text-red-900">{answers[q.id] ? (<><span className="font-black mr-2">{answers[q.id]}.</span>{q[`option_${answers[q.id].toLowerCase()}`] || "Option text unavailable"}</>) : "Skipped"}</p></div>)}</div>{isPremium && q.explanation && <div className="mt-4 pt-4 border-t border-gray-100"><p className="text-[9px] font-black text-blue-600 uppercase tracking-widest mb-1">Concept Brief</p><p className="text-xs text-gray-600 leading-relaxed bg-blue-50/30 p-3 rounded-xl border border-blue-100 italic">{q.explanation}</p></div>}</div>))}</div>) : (<div className="bg-[#0a0a0a] rounded-[2.5rem] shadow-2xl border border-yellow-900/30 overflow-hidden min-h-[500px] relative group">{!isPremium ? (<div className="absolute inset-0 z-10 bg-gradient-to-b from-black via-[#0a0a0a] to-[#1a1a1a] flex flex-col items-center justify-center text-center p-10"><div className="relative mb-10"><div className="absolute inset-0 bg-yellow-500 blur-[80px] opacity-20 group-hover:opacity-40 transition-opacity animate-pulse"></div><div className="w-28 h-28 bg-gradient-to-br from-yellow-400 via-orange-600 to-yellow-700 rounded-[2.5rem] flex items-center justify-center relative z-10 shadow-[0_0_50px_rgba(234,179,8,0.3)] transform group-hover:scale-110 transition-transform duration-700"><Lock size={48} className="text-white drop-shadow-2xl" strokeWidth={2.5} /></div></div><div className="relative z-10"><h3 className="text-3xl font-black text-white mb-4 tracking-tighter uppercase italic">Confidential Briefing</h3><p className="text-gray-400 text-sm mb-12 max-w-xs font-medium leading-relaxed">Your cognitive performance data is locked. Access the <span className="text-yellow-500 font-bold">AI Tactical Roadmap</span> to secure your success.</p><button onClick={() => setShowUpgrade(true)} className="bg-yellow-500 text-black px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.3em] shadow-[0_10px_40px_rgba(234,179,8,0.4)] hover:bg-white hover:scale-105 transition-all active:scale-95">Unlock The Vault</button></div><div className="absolute bottom-6 left-0 w-full text-center opacity-5 pointer-events-none"><p className="text-[40px] font-black uppercase tracking-[0.5em] whitespace-nowrap">CLASSIFIED • CLASSIFIED • CLASSIFIED</p></div></div>) : (<div className="p-0">{!analysis ? (<div className="text-center py-32 px-8 bg-white"><div className="w-16 h-16 bg-purple-50 rounded-full flex items-center justify-center mx-auto mb-6 animate-pulse"><BrainCircuit size={40} className="text-purple-600" /></div><h3 className="font-black text-gray-900 text-xs uppercase tracking-widest mb-2">Analyzing Performance</h3><p className="text-gray-400 text-[10px] mb-8 uppercase tracking-widest">Crafting personalized recovery roadmap...</p><button onClick={generateAnalysis} disabled={analyzing} className="bg-purple-900 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all">{analyzing ? "PROCESSING..." : "GENERATE REPORT"}</button></div>) : (<div className="bg-[#fcfcfc] min-h-[600px] animate-in fade-in duration-700"><div className="bg-purple-900 text-white p-10 pb-12 rounded-t-[2.5rem] shadow-lg relative overflow-hidden"><div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-20 -mt-20 blur-3xl"></div><div className="relative z-10 text-left"><div className="flex items-center gap-3 mb-4"><div className="bg-white/20 p-2 rounded-lg backdrop-blur-md"><Sparkles size={20} /></div><span className="text-[10px] font-black uppercase tracking-[0.3em] text-purple-200">Intelligence Briefing</span></div><h3 className="text-3xl font-black uppercase tracking-tighter leading-none">Tactical Brief</h3><p className="text-purple-300 text-xs font-bold uppercase tracking-widest mt-3 opacity-80">Personalized Recovery Plan for {student?.name}</p></div></div><div className="p-8 md:p-12 text-left border-l-[8px] border-l-purple-600 bg-white"><div className="text-gray-800 leading-relaxed font-medium space-y-6"><ReactMarkdown components={{ h1: ({children}) => <h1 className="text-xl font-black text-gray-900 uppercase tracking-widest border-b border-purple-100 pb-2 mt-8 first:mt-0">{children}</h1>, h2: ({children}) => <h2 className="text-lg font-bold text-purple-900 mt-6 mb-3">{children}</h2>, p: ({children}) => <p className="text-sm text-gray-600 mb-4 leading-7">{children}</p>, ul: ({children}) => <ul className="list-disc pl-5 space-y-2 text-sm text-gray-700">{children}</ul>, li: ({children}) => <li className="pl-1">{children}</li>, strong: ({children}) => <strong className="font-black text-purple-800">{children}</strong> }}>{analysis}</ReactMarkdown></div><div className="mt-16 pt-8 border-t border-gray-50 flex items-center justify-between opacity-40"><div className="flex items-center gap-2"><BrainCircuit size={14} /><span className="text-[8px] font-black uppercase tracking-widest">ExamForge AI Engine</span></div><span className="text-[8px] font-black uppercase tracking-widest">Classified Document</span></div></div></div>)}</div>)}</div>)}</div>
      </main>
    );
  }
  ...
  return (
    <main className="h-screen flex flex-col bg-[#f0f2f5] font-sans overflow-hidden select-none relative">
      ...
      <header className="h-14 bg-[#004d00] text-white flex justify-between items-center px-4 shrink-0 z-[160] border-b border-green-800"><div className="flex items-center gap-3"><div className="w-8 h-8 bg-white rounded-lg flex items-center justify-center text-[#004d00] font-black text-sm shadow-inner">{student?.name?.charAt(0).toUpperCase()}</div><div className="leading-tight"><h1 className="font-black text-[10px] uppercase tracking-widest text-green-100 truncate w-24">{student?.name}</h1><p className="text-[9px] font-mono opacity-70 tracking-tighter uppercase">{course?.code}</p></div></div><div className="flex items-center gap-3"><div className={`flex items-center gap-2 bg-black/30 px-3 py-1.5 rounded-full border border-white/10 ${timeLeft < 300 ? 'animate-pulse bg-red-900/50 border-red-500' : ''}`}><Clock size={12} className={timeLeft < 300 ? "text-red-500" : "text-green-400"} /><span className={`font-mono font-black text-sm tracking-widest ${timeLeft < 300 ? "text-red-500" : "text-white"}`}>{formatTime(timeLeft || 0)}</span></div><button onClick={() => setShowSubmitModal(true)} className="bg-red-600 text-white px-4 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">Submit</button></div></header>
      <div className="flex-1 flex flex-col p-4 overflow-hidden relative z-10"><div className="flex-1 bg-white rounded-[2.5rem] shadow-2xl shadow-green-900/5 border border-gray-100 p-6 flex flex-col justify-between relative overflow-hidden"><div className="flex justify-between items-center mb-4 shrink-0"><span className="font-black text-green-900 text-[9px] tracking-[0.2em] uppercase bg-green-50 px-2 py-1 rounded-lg border border-green-100">Q {String(currentQIndex + 1).padStart(2, '0')} / {questions.length}</span><span className="text-[9px] font-black text-gray-300 uppercase tracking-widest">{marksPerQuestion} Marks</span></div><div className="flex-1 flex flex-col justify-center py-4 overflow-y-auto custom-scrollbar pr-2"><h2 className="text-base md:text-xl font-bold text-gray-900 leading-relaxed text-left">{currentQ.question_text}</h2></div><div className="grid grid-cols-1 gap-2.5 mt-4 shrink-0">{['A','B','C','D'].map((opt) => (<button key={opt} onClick={() => handleSelect(opt)} className={`group p-4 rounded-3xl border-2 text-left transition-all duration-200 flex items-center gap-4 active:scale-[0.98] ${answers[currentQ.id] === opt ? 'border-green-600 bg-green-50 ring-2 ring-green-100' : 'border-gray-100 bg-white'}`}><span className={`shrink-0 w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs border transition-colors ${answers[currentQ.id] === opt ? 'bg-green-600 text-white border-green-600' : 'bg-gray-50 text-gray-400 border-gray-200 group-hover:bg-white'}`}>{opt}</span><span className={`font-bold text-xs leading-tight ${answers[currentQ.id] === opt ? 'text-green-900 font-black' : 'text-gray-600'}`}>{currentQ[`option_${opt.toLowerCase()}`]}</span></button>))}</div></div></div>
