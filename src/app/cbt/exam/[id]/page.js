...
// 1. KEEP THE WORKING RELATIVE IMPORT
const UpgradeModal = dynamic(() => import("../../../../components/cbt/UpgradeModal"), { ssr: false });

/* === MALPRACTICE OVERLAY (CSS ONLY, NO HEAVY LIBS) === */
function MalpracticeOverlay({ count }) {
  return (
    <div className="fixed inset-0 z-[999] bg-red-900/95 flex flex-col items-center justify-center text-white animate-pulse">
      ...
      <h2 className="text-4xl font-black uppercase tracking-widest text-center">Malpractice<br/>Detected</h2>
...
/* === CUSTOM MODAL (KEPT FROM WORKING CODE) === */
...
export default function ExamPage() {
  ...
  // 1. IRON MODE SECURITY INJECTION
  useEffect(() => {
    if (!mounted || isSubmitted) return;

    // A. Disable Right Click & Copy
  ...
  // 2. Initial Load (KEPT EXACTLY AS WORKING CODE)
  useEffect(() => {
    setMounted(true);
    ...
    async function loadExam() {
      try {
        ...
        setQuestions(data.questions || []); // Safety check
  ...
  // 3. Submit Logic (KEPT AS WORKING CODE)
  ...
  // 4. Timer (KEPT AS WORKING CODE + FROZEN CHECK)
  useEffect(() => {
  ...
  const getGridColor = (index, qId) => {
    if (currentQIndex === index) return "bg-[#004d00] text-white border-[#004d00] ring-2 ring-green-200"; // JAMB Green
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center bg-white text-green-800 font-bold text-lg animate-pulse">Initializing Iron Mode...</div>;
  ...
  // === RESULT VIEW (KEPT AS WORKING CODE) ===
  if (isSubmitted) {
  ...
  return (
    // ADDED: select-none for Anti-Copy
