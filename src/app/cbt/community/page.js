...
import { Send, Users, ShieldAlert, Trash2, Building2, Sparkles, Megaphone } from "lucide-react";
...
export default function CommunityPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => fetchFeed(parsed.department), 3000); // 3s Heartbeat
  ...
  const handleDelete = async (postId) => {
    if (!confirm("COMMANDER: Delete this intel permanently?")) return;
    try {
  ...
  return (
    <main className="min-h-screen bg-[#f2f4f7] pb-32 font-sans">
      {/* === GLASS HEADER === */}
      <header className="fixed top-0 left-0 right-0 bg-[#004d00]/90 backdrop-blur-xl text-white pt-12 pb-6 px-6 rounded-b-[2.5rem] shadow-2xl z-50 border-b border-white/10">
        <div className="flex justify-between items-center max-w-2xl mx-auto">
          <div>
            <h1 className="text-xl font-black uppercase tracking-widest flex items-center gap-2">
              <Users size={22} className="text-green-300" /> Town Hall
            </h1>
            <p className="text-[10px] text-green-200 font-bold uppercase tracking-wider mt-1 flex items-center gap-1">
              <Building2 size={10} /> {isAdmin ? "COMMAND CENTER" : student.department || "General Campus"}
            </p>
          </div>
      ...
      <div className="pt-40 px-4 max-w-2xl mx-auto">
        ...
        <div className={`bg-white p-1 rounded-[2rem] shadow-xl border mb-8 transition-all ${isAdmin ? 'border-red-100 shadow-red-900/10' : 'border-green-100 shadow-green-900/5'}`}>
          <div className="bg-gray-50 rounded-[1.8rem] p-4">
            <textarea 
              ...
              placeholder={isAdmin ? "Type a Global Broadcast..." : "Share intel with your department..."}
          ...
          <div className="flex justify-between items-center px-4 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] font-black text-gray-400 uppercase tracking-widest">
                {isAdmin ? "SECURE CHANNEL" : "LIVE FEED"}
              </span>
            </div>
            <button 
              onClick={handlePost} 
            ...
            >
              {posting ? "Sending..." : (isAdmin ? "BROADCAST" : "POST")} {isAdmin ? <Megaphone size={12} /> : <Send size={12} />}
            </button>
          </div>
        </div>
        ...
        <div className="space-y-5 pb-10">
          {loading && <div className="text-center text-gray-400 text-[10px] font-black uppercase animate-pulse mt-10 tracking-widest">Establishing Uplink...</div>}
          ...
          {posts.map((post) => (
            <div key={post.id} className={`p-5 rounded-[2rem] shadow-sm border relative group transition-all hover:-translate-y-1 duration-300 ${post.is_announcement ? 'bg-gradient-to-br from-red-50 to-white border-red-100 shadow-red-100' : 'bg-white border-gray-100'}`}>
              ...
              {/* ADMIN DELETE BUTTON (Only visible to YOU) */}
              {isAdmin && (
                <button 
              ...
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-2xl flex items-center justify-center font-black text-sm text-white shadow-md overflow-hidden ${post.is_admin ? 'bg-red-600' : 'bg-gray-900'}`}>
                    {post.is_admin ? <ShieldAlert size={20} /> : <img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${post.author_name.replace(/\s/g, '')}`} alt="Avatar" className="w-full h-full object-cover" />}
                  </div>
                  <div>
                    <h3 className="font-black text-xs text-gray-900 uppercase flex items-center gap-1.5">
                      {post.author_name} 
                      {post.is_admin && <span className="bg-red-100 text-red-600 px-1.5 py-0.5 rounded text-[8px] tracking-wider">COMMANDER</span>}
                    </h3>
                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-wide flex items-center gap-1">
                      {post.is_admin ? "Global Alert" : post.department || "General"} <span className="w-1 h-1 bg-gray-300 rounded-full"></span> {new Date(post.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                    </p>
                  </div>
                </div>
              </div>
          ...
          {!loading && posts.length === 0 && (
            <div className="text-center py-12 opacity-40">
              ...
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Channel Silent.</p>
            </div>
          )}
        </div>
