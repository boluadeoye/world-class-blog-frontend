      ...
      <header className="fixed top-0 left-0 right-0 bg-[#004d00]/95 backdrop-blur-xl text-white pt-12 pb-6 px-6 rounded-b-[2.5rem] shadow-2xl z-40 border-b border-white/10">
        <div className="flex justify-between items-center max-w-2xl mx-auto">
          <div>
            ...
            <p className="text-[10px] text-green-100 font-bold uppercase tracking-wider mt-1">
              {isAdmin ? "ADMINISTRATOR" : `${student.department || "General"} Community`}
            </p>
          </div>
        ...
        <div className={`bg-white p-1.5 rounded-[2rem] shadow-xl border mb-8 ${isAdmin ? 'border-red-200' : 'border-green-100'}`}>
          <div className="bg-gray-50 rounded-[1.8rem] p-4">
            <textarea 
              ...
              placeholder={isAdmin ? "Broadcast an official announcement to everyone..." : "Ask a question or share an update with your department..."} 
              className="w-full bg-transparent text-sm font-medium focus:outline-none resize-none h-24 text-gray-900 placeholder:text-gray-400" 
            />
          </div>
          <div className="flex justify-between items-center px-4 py-3">
            <span className="text-[9px] font-black uppercase tracking-widest text-gray-400">
              {isAdmin ? "OFFICIAL BROADCAST" : "NEW POST"}
            </span>
            <button 
              onClick={handlePost} 
              disabled={posting || !content.trim()} 
              className={`px-6 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 shadow-lg active:scale-95 disabled:opacity-50 ${isAdmin ? 'bg-red-600 text-white' : 'bg-[#004d00] text-white'}`}
            >
              {posting ? "..." : (isAdmin ? "ANNOUNCE" : "POST")} <Send size={12} />
            </button>
          </div>
        </div>
        ...
        <div className="space-y-5">
          ...
          {posts.map((post) => (
            <div key={post.id} onClick={() => openThread(post)} className={`p-6 rounded-[2rem] shadow-sm border relative transition-all active:scale-[0.98] cursor-pointer ${post.is_announcement ? 'bg-gradient-to-br from-[#2b0a0a] to-[#4a0f0f] border-red-900 text-white shadow-red-900/20' : 'bg-white border-gray-100 text-gray-800'}`}>
              ...
              {isAdmin && (
                <button onClick={(e) => handleDeletePost(e, post.id, 'post')} className="absolute top-4 right-4 p-2 bg-white/10 rounded-full text-white/50 hover:bg-red-600 hover:text-white transition-colors z-20">
                  <Trash2 size={14} />
                </button>
              )}

              <div className="flex items-center gap-3 mb-4">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-sm shadow-md overflow-hidden border-2 ${post.is_admin ? 'bg-red-600 border-red-400' : 'bg-gray-50 border-gray-100'}`}>
                  {post.is_admin ? <Megaphone size={20} /> : <img src={`https://api.dicebear.com/7.x/notionists/svg?seed=${post.author_name.replace(/\s/g, '')}`} className="w-full h-full object-cover" />}
                </div>
                <div>
                  <h3 className={`font-black text-xs uppercase flex items-center gap-1.5 ${post.is_announcement ? 'text-red-100' : 'text-gray-900'}`}>
                    {post.author_name} 
                    {post.is_premium && <BadgeCheck size={14} className="text-blue-400 fill-blue-400 text-white" />}
                  </h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase ${post.is_admin ? 'bg-red-500/20 text-red-200' : 'bg-gray-100 text-gray-500'}`}>
                      {post.is_admin ? "ADMIN" : post.department || "General"}
                    </span>
        ...
        <div className="fixed inset-0 z-[100] bg-black/80 backdrop-blur-sm flex items-end justify-center">
          <div className="bg-[#f4f6f8] w-full max-w-lg h-[85vh] rounded-t-[2.5rem] flex flex-col overflow-hidden shadow-2xl animate-in slide-in-from-bottom duration-300">
            <div className="bg-white p-5 border-b flex justify-between items-center">
              ...
              <button onClick={() => setActivePost(null)} className="p-2 bg-gray-200 text-gray-700 rounded-full hover:bg-red-50 hover:text-red-600 transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-5 custom-scrollbar">
              <div className="bg-white p-5 rounded-[2rem] border border-gray-100 mb-6">
                <h4 className="font-black text-xs uppercase mb-2 flex items-center gap-2 text-gray-900">
                  ...
                  {activePost.is_premium && <BadgeCheck size={12} className="text-blue-400" />}
                </h4>
              ...
              <div className="space-y-3">
                {comments.map(c => (
                  <div key={c.id} className="bg-white p-4 rounded-2xl border border-gray-50 relative">
                    {isAdmin && (
                      <button onClick={() => handleDelete(c.id, 'comment')} className="absolute top-3 right-3 text-gray-300 hover:text-red-500">
                        <Trash2 size={14} />
                      </button>
                    )}
            ...
            <div className="p-4 bg-white border-t flex gap-2">
              <input 
                ...
                className="flex-1 bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500" 
              />
