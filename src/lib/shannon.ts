...
export async function getRotatedKey(): Promise<ShannonKey | null> {
  try {
    // 1. Reset expired keys (4h window for calls, 24h for tokens)
    await sql`
      UPDATE shannon_api_pool 
      SET calls_left = 100, reset_at = (CURRENT_TIMESTAMP + interval '4 hours')
      WHERE reset_at < CURRENT_TIMESTAMP;
    `;

    await sql`
      UPDATE shannon_api_pool 
      SET tokens_left = 80000, daily_reset_at = (CURRENT_TIMESTAMP + interval '24 hours')
      WHERE daily_reset_at < CURRENT_TIMESTAMP;
    `;

    // 2. Pick the freshest available key
    const keys = await sql`
      SELECT id, key_val FROM shannon_api_pool 
      WHERE is_active = TRUE 
      AND calls_left > 2 
      AND tokens_left > 4000
    ...
    // 3. Optimistic Update (Decrement immediately)
    await sql`
      UPDATE shannon_api_pool 
...
export async function updateTokenUsage(id: number, tokensUsed: number) {
  try {
    await sql`
      UPDATE shannon_api_pool 
      SET tokens_left = tokens_left - ${tokensUsed}
      WHERE id = ${id}
    `;
  } catch (err) {
