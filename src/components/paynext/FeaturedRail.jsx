...
export default function FeaturedRail({ posts = [] }) {
  ...
  useEffect(() => {
    ...
    if (reduce) return;
    let raf = 0, dir = 1, speed = 0.45, paused = false, timer = 0;
    const loop = () => {
      if (!paused) {
    ...
    const pause = () => { paused = true; clearTimeout(timer); timer = setTimeout(()=>paused=false, 1500); };
    el.addEventListener("pointerdown", pause, { passive:true });
    el.addEventListener("wheel", pause, { passive:true });
    el.addEventListener("touchstart", pause, { passive:true });
    raf = requestAnimationFrame(loop);
    return () => { cancelAnimationFrame(raf); clearTimeout(timer);
      el.removeEventListener("pointerdown", pause); el.removeEventListener("wheel", pause); el.removeEventListener("touchstart", pause); };
  }, []);

  // Filter out any video posts defensively
  const items = (Array.isArray(posts) ? posts : []).filter(p => (p?.type || "article") !== "video");
