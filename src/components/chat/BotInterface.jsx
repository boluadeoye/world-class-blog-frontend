...
export default function BotInterface({ blogContext }) {
  const [messages, setMessages] = useState([]);
  const [input, setInput] = useState("");
  const [isLoading, setIsLoading] = useState(false);
  const scrollRef = useRef(null);

  // 1. AGGRESSIVE LOAD (New Key)
  useEffect(() => {
    const saved = localStorage.getItem("bolu_chat_history_v8"); 
    if (saved) {
  ...
  useEffect(() => {
    if (messages.length > 0) {
      localStorage.setItem("bolu_chat_history_v8", JSON.stringify(messages));
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-950 relative font-sans">
      ...
      <div className="flex items-center justify-between px-6 py-4 bg-slate-900/50 backdrop-blur-md border-b border-white/5 z-20">
        ...
        <button onClick={() => { localStorage.removeItem("bolu_chat_history_v8"); initChat(); }} className="text-xs text-slate-500 hover:text-red-400">Reset</button>
      </div>
