...
import { Send, Trash2, Bot, User, ArrowLeft, Sparkles } from "lucide-react";
...
export default function ChatInterface({ blogContext }) {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-slate-900 md:rounded-3xl border-x md:border-y border-white/10 shadow-2xl overflow-hidden">
      
      {/* === HEADER: Digital Consciousness === */}
      <div className="flex items-center justify-between px-5 py-4 bg-slate-950 border-b border-white/10 z-20">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              <div className="w-10 h-10 rounded-full bg-gradient-to-br from-slate-800 to-black border border-white/10 flex items-center justify-center">
                <Bot size={18} className="text-indigo-400" />
              </div>
              {/* Green Active Light */}
              <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 rounded-full border-2 border-slate-950 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.6)]"></div>
            </div>
            <div>
      ...
      <div className="p-4 bg-slate-950 border-t border-white/10">
        <form onSubmit={handleSend} className="relative flex items-center gap-2">
          <input 
            ...
            className="flex-1 bg-slate-900 text-white placeholder-slate-500 px-5 py-3.5 rounded-full border border-white/10 focus:border-indigo-500/50 focus:outline-none text-sm transition-all shadow-inner"
            autoFocus
          />
          <button 
            type="submit" 
            disabled={!input.trim() || isTyping}
            className="p-3.5 rounded-full bg-indigo-600 hover:bg-indigo-500 text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-indigo-500/20"
          >
            <Send size={18} />
          </button>
        </form>
