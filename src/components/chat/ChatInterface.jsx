...
import { Send, Trash2, Bot, Sparkles, MessageCircle, Code, Briefcase, CreditCard } from "lucide-react";

const SUGGESTIONS = [
  { label: "Build me an AI Agent", icon: Bot, prompt: "I want an AI Agent for my business. How does it work?" },
  { label: "View Projects", icon: Briefcase, prompt: "Show me your recent engineering projects." },
  { label: "Contact Bolu", icon: MessageCircle, prompt: "How can I contact you directly?" },
];
...
export default function ChatInterface({ blogContext }) {
  ...
  useEffect(() => {
    setMounted(true);
    const saved = localStorage.getItem("bolu_chat_history_v5");
    if (saved) {
  ...
  useEffect(() => {
    if (mounted && messages.length > 0) {
      localStorage.setItem("bolu_chat_history_v5", JSON.stringify(messages));
  ...
  const initChat = () => {
    setMessages([{
      id: "init",
      role: "assistant",
      content: "System Online. I am Bolu's Digital Consciousness. I can discuss engineering, negotiate contracts, or help you build your own AI infrastructure. How may I assist?"
    }]);
  };

  const clearChat = () => {
    localStorage.removeItem("bolu_chat_history_v5");
    initChat();
  };
  ...
  const renderMessage = (text) => {
    ...
    if (text.includes("paystack.com")) {
      const parts = text.split(/(https:\/\/paystack\.com\/[^\s]+)/);
      return (
        <div className="space-y-3">
          <p>{parts[0]}</p>
          {parts[1] && (
            <a href={parts[1]} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-4 bg-emerald-900/30 border border-emerald-500/30 rounded-xl hover:bg-emerald-900/50 transition-all group">
              <div className="p-2 bg-emerald-500/20 rounded-lg text-emerald-400 group-hover:scale-110 transition-transform">
                <CreditCard size={20} />
              </div>
              <div>
                <div className="text-emerald-300 font-bold text-sm">Secure Payment Link</div>
                <div className="text-emerald-500/60 text-xs">Paystack Checkout</div>
              </div>
            </a>
          )}
          <p>{parts[2]}</p>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-6 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            <div className={`max-w-[85%] p-3.5 rounded-2xl text-sm leading-relaxed shadow-lg backdrop-blur-sm ${msg.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none shadow-indigo-500/10' : msg.isError ? 'bg-red-900/20 text-red-200 border border-red-500/20' : 'bg-slate-800/90 text-slate-200 border border-white/5 rounded-tl-none'}`}>{renderMessage(msg.content)}</div>
            {msg.role === 'user' && <div className="w-8 h-8 rounded-full bg-slate-800 border border-white/10 flex items-center justify-center shrink-0 mt-1"><User size={14} className="text-slate-400" /></div>}
          </div>
        ))}
