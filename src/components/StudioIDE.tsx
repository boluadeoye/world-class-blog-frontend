...
function balanceMarkdown(markdown: string): string {
  ...
  const openCodeBlocks = codeBlockMatches ? codeBlockMatches.length % 2 !== 0 : false;
  
  // Balance Bold (**)
  const boldMatches = markdown.match(/\*\*/g);
  const openBold = boldMatches ? boldMatches.length % 2 !== 0 : false;

  // Balance Italic (*)
  const cleanItalicText = markdown.replace(/\*\*/g, '');
  const italicMatches = cleanItalicText.match(/\*/g);
  const openItalic = italicMatches ? italicMatches.length % 2 !== 0 : false;

  if (openItalic) balanced += '*';
  if (openBold) balanced += '**';
  if (openCodeBlocks) balanced += '\n```';
      ...
      while (boundary !== -1) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            console.warn("Partial block received, retaining context:", data, e);
          }
        }
...
export default function StudioIDE({ initialSession }: any) {
  ...
  // Synchronize dynamic heights to prevent layout collapsing under mobile viewports
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      document.documentElement.style.setProperty('--keyboard-offset', `${window.innerHeight - height}px`);
    };
  ...
  // Sync state cleanly when switching active session IDs, bypassing duplicate arrays
  useEffect(() => {
      ...
      if (err.name !== "AbortError") {
        startTransition(() => {
          setMessages([...optimisticMessages, { 
            id: generateUUID(), 
            role: "system_error", 
            content: `**SYSTEM ALERT:** ${err.message}`, 
            createdAt: new Date().toISOString() 
          }]);
          setIsThinking(false);
          setIsStreaming(false);
  ...
  return (
    <div 
      className="fixed inset-0 bg-[#000000] text-[#e3e3e3] overflow-hidden antialiased flex"
      ...
      <div className={`fixed inset-y-0 left-0 w-[85vw] max-w-[320px] bg-[#0b0b0b] border-r border-white/5 z-50 transform transition-transform duration-300 ease-out flex flex-col ${leftOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {sessions.map(s => (
            <button 
              key={s.id} 
              onClick={() => { window.location.href = `/studio/${s.id}`; }} 
              className={`w-full text-left px-4 py-3 rounded-md text-[13px] truncate mb-1 transition-all ${activeId === s.id ? "bg-emerald-500/10 text-emerald-400 font-medium" : "text-white/40 hover:bg-white/5"}`}
            >
              {s.title}
            </button>
          ))}
      ...
      <div className="flex-1 flex flex-col h-full w-full relative overflow-hidden">
        ...
        <div className="flex-1 min-h-0 relative w-full overflow-hidden flex flex-col">
          <MessageFeed 
            ...
            streamingContent={balancedStreamingContent} 
          />
        </div>
        ...
        <div className="w-full p-4 bg-gradient-to-t from-[#000000] via-[#000000]/95 to-transparent pt-6 z-30 shrink-0">
          <div className="max-w-3xl mx-auto bg-[#0b0b0b] rounded-lg p-1.5 flex items-end gap-1 border border-white/5 shadow-2xl focus-within:border-emerald-500/30 transition-all">
            ...
            <textarea 
              ...
              className="flex-1 bg-transparent border-none outline-none text-[14px] px-2 py-3 placeholder:text-white/20 resize-none max-h-[150px] font-sans"
            />
            <button 
              onClick={isStreaming ? () => abortRef.current?.abort() : handleSubmit} 
              disabled={!isStreaming && !inputValue.trim()} 
              className={`flex-shrink-0 w-10 h-10 rounded-md flex items-center justify-center transition-all mb-0.5 ${isStreaming ? 'bg-white/10' : inputValue.trim() ? 'bg-emerald-600 shadow-[0_0_20px_rgba(16,185,129,0.4)]' : 'bg-white/5 text-white/10'}`}
            >
      ...
      <div className={`fixed inset-y-0 right-0 w-[85vw] max-w-[320px] bg-[#0b0b0b] border-l border-white/5 z-50 transform transition-transform duration-300 ease-out flex flex-col ${rightOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="p-4 flex-1">
          <textarea 
            ...
            className="w-full h-full p-4 rounded-md bg-[#000000] border border-white/5 text-[12px] font-mono text-white/50 leading-relaxed resize-none outline-none focus:border-emerald-500/50"
          />
        </div>
      </div>
