...
/* === ARTICLE HERO (Cinematic Entrance) === */
export function ArticleHero({ post }) {
  ...
  // Calculate time dynamically based on content length
  const readTime = getReadingTime(post.content);

  return (
    <section className="relative pt-36 pb-20 px-6 overflow-hidden flex flex-col items-center text-center">
      ...
      <div className="relative z-10 max-w-4xl mx-auto">
        ...
        <motion.div 
          ...
          className="inline-flex flex-wrap items-center justify-center gap-8 text-sm text-slate-400 border-y border-white/5 py-5 px-10 bg-white/[0.02] backdrop-blur-sm rounded-full"
        >
          ...
          <div className="w-1 h-1 rounded-full bg-slate-600"></div>
          <div className="flex items-center gap-2"><Clock size={14} className="text-amber-500/80" /><span>{readTime}</span></div>
        </motion.div>
      </div>
    </section>
  );
